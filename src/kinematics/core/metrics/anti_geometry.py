...
(wheel contact center for outboard brakes, wheel center for inboard-sprung
...
def calculate_svsa_angle(ctx: MetricContext) -> float | None:
    ...
    contact center to the SVIC is resolved in the chassis XZ plane, using the
    ...
    if svic is None:
        return None
    contact_center = ctx.wheel_contact_center
    ...
    run = float(svic[Axis.X]) - float(contact_center[Axis.X])
    if abs(run) < EPS_GEOMETRIC:
        # Vertical side-view line: slope undefined.
        return None
    rise = float(svic[Axis.Z]) - float(contact_center[Axis.Z])
...
def calculate_anti_dive_pct(ctx: MetricContext) -> float | None:
    ...
    center-to-SVIC line. With L the wheelbase, h the CG height above road, and
    n the road plane's upward normal, the line inclination is taken about the
    contact center as:

        tan_theta = n . (SVIC - T) / (T_x - SVIC_x)

    The contact center T lies on the road plane, so the rise n . (SVIC - T) is the
    ...
    layout (SVIC above and BEHIND the front wheel contact center). Then:
    ...
    # Run measured from SVIC to the contact center so tan_theta is positive
    # when the SVIC sits behind (-X of) it: the anti-dive geometry.
    run = float(ctx.road.forward.dot(contact_center - svic))
    if abs(run) < EPS_GEOMETRIC:
        return None
    ...
    # The contact center lies on the road plane, so its rise to the SVIC along
...
def calculate_anti_lift_pct(ctx: MetricContext) -> float | None:
    ...
    rear suspension reacts the brake force along the wheel-contact-center to
    SVIC line. With n the road plane's upward normal, the line inclination is
    taken about the contact center as:
    ...
    and AHEAD (+X) of the rear wheel contact center. Then:
    ...
    run = float(ctx.road.forward.dot(svic - contact_center))
    if abs(run) < EPS_GEOMETRIC:
        return None
    height = _cg_height_above_road(ctx)
    if height is None:
...
def calculate_anti_squat_pct(ctx: MetricContext) -> float | None:
    ...
    not the wheel contact center line. The full drive torque is carried by
