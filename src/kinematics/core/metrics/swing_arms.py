...
positions and the wheel contact center. The ISO 8855 vehicle-axis orientation
...
SVSA (Side-View Swing Arm):
    The horizontal (X-axis) distance from the wheel contact center point to
    the side-view instant center (SVIC).

        SVSA = SVIC_X - ContactCenter_X

    Positive when the SVIC is ahead of (+X relative to) the wheel contact
    center, which is the typical case for a conventional double-wishbone
    layout. Negative values indicate the SVIC is behind that contact center.

FVSA (Front-View Swing Arm):
    The Euclidean distance in the YZ plane from the wheel contact center point
    to the front-view instant center (FVIC), with a sign that encodes
    whether the FVIC is inboard or outboard of the contact center.

        FVSA = +/- sqrt((FVIC_Y - T_Y)^2 + (FVIC_Z - T_Z)^2)

    Positive when the FVIC is inboard (closer to vehicle centerline) of the
    contact center, where inboard/outboard is judged by the component of
    ...
    FVIC_Y - T_Y.
    Negative values indicate the FVIC is outboard of the contact center.
"""
...
def calculate_svsa_length(ctx: MetricContext) -> float | None:
    ...
    The displacement from the wheel contact center to the SVIC is expressed
    ...
    center; negative means behind. World-space presentation is not used.
    ...
    if svic is None:
        return None
    displacement = svic - ctx.wheel_contact_center
...
def calculate_fvsa_length(ctx: MetricContext) -> float | None:
    ...
    The displacement from the wheel contact center to the FVIC is expressed
    ...
    if fvic is None:
        return None
    displacement = fvic - ctx.wheel_contact_center
    ...
    # of the contact center. The road lateral direction points from vehicle
