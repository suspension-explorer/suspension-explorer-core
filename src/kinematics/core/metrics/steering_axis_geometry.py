"""Source-agnostic steering-axis geometry and common alignment metrics."""
...
class SteeringAxis:
    ...
    :meth:`from_unoriented_line`. Once established, every source flows through
    the same intersection and metric functions in this module.
    """
...
def calculate_scrub_radius(
    axis: SteeringAxis,
    road: RoadPlane,
    wheel_contact_centre: Point3,
) -> float | None:
    """Return unsigned road-plane scrub radius for ``axis``."""
    displacement = _road_displacement(axis, road, wheel_contact_centre)
...
def _road_displacement(
    ...
    if intersection is None or not np.isfinite(wheel_contact_centre.data).all():
        return None
    return intersection - wheel_contact_centre
