...
class SuspensionState(Generic[KeyT]):
    ...
    The point-key type is generic
    (:data:`~kinematics.core.primitives.point_ref.PointKey`): single-corner models
    key on ``PointID``, axle models key on ``PointRef``. The keys of a single state
    must be homogeneous so that ``free_points`` can be sorted for a stable variable
    ordering.
