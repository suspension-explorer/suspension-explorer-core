...
def rotation_about_fixed_axis_deg(
    ...
    """
    Signed rotation (degrees) of a point about a chassis-fixed axis, from
    its design position to its current position.

    Dual-safe replica of vector_utils.geometric.signed_angle_about_axis for
    the case where the axis and the design position are constants (zero
    derivative): the rotation of the current radius vector r1 relative to
    the constant design radius r0, right-hand-rule positive about
    axis_direction:

        angle = atan2(d dot (r0 cross r1), r0_perp dot r1_perp)

    Args:
        positions: Position mapping (plain or dual).
        point_id: The rotating point.
        design_position: The point's design-condition position (constant).
        axis_point: Any point on the axis (constant).
        axis_direction: Unit axis direction (constant, sets positive sense).

    Returns:
        Signed rotation in degrees (float or DualScalar).
    """
    r0 = np.asarray(design_position, dtype=np.float64) - axis_point
    r1 = _vec(positions, point_id) - axis_point

    # Components perpendicular to the axis. The scalar-first product order
    # (DualScalar * ndarray) keeps the result on the dual substrate.
    r0_perp = r0 - np.dot(r0, axis_direction) * axis_direction
    r1_perp = r1 - dot(r1, axis_direction) * axis_direction

    sin_term = dot(axis_direction, cross(r0, r1))
    cos_term = dot(r0_perp, r1_perp)
    return degrees(atan2(sin_term, cos_term))
