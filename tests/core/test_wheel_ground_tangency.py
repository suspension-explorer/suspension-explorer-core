...
def _solve(
    ...
    keys = _keys()
    return solve_axle_wheel_contact_centers(
        positions,
...
def test_seed_from_contact_centers_recovers_the_lateral_contact_angle():
    left = Point3((0.0, 620.0, -12.0))
    right = Point3((0.0, -580.0, 18.0))
    lateral_separation = 620.0 - (-580.0)
    vertical_separation = -12.0 - 18.0

    seed = seed_from_contact_centers(left, right)
    reversed_seed = seed_from_contact_centers(right, left)
...
def test_seed_from_contact_centers_matches_the_solved_ground_normal_angle():
    tangency = _solve(_positions())

    seed = seed_from_contact_centers(tangency.left, tangency.right)

    assert seed == pytest.approx(tangency.normal_angle)


def test_seed_from_contact_centers_returns_none_for_unusable_points():
    collapsed = seed_from_contact_centers(
        Point3((0.0, 100.0, 10.0)), Point3((0.0, 100.0, -10.0))
    )
    non_finite = seed_from_contact_centers(
...
def test_axle_derived_spec_omits_the_coupled_tangents() -> None:
    ...
    for side in (Side.LEFT, Side.RIGHT):
        tangent = PointRef(side, PointID.WHEEL_CONTACT_CENTER)
        assert tangent not in spec.functions, (
            "Coupled wheel contact centers are closure outputs, not derived points"
        )
...
def test_ground_closure_recovers_its_seed_from_the_stored_tangents(
    ...
    state = axle.initial_state()
    positions = dict(state.positions)
    stored_seed = seed_from_contact_centers(
