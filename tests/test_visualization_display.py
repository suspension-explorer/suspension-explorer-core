...
from kinematics.core.topology import LinkRole
...
def test_corner_rocker_fan_becomes_axis_and_perpendicular_arms(
    ...
    groups = rocker_display_groups(topology)
    positions = display_positions(
        suspension.initial_state().positions,
        display_point_keys(topology),
        groups,
    )
    links = display_links(topology)

    assert len(groups) == 1
    assert all(isinstance(link.role, LinkRole) for link in links)
    assert not any(hasattr(link, "color") for link in topology.links)
